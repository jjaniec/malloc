...
/*   Updated: 2019/10/18 19:25:17 by jjaniec          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void basic_tests()
{
	...
	tmp = malloc(TINY_MAX_SIZE / 2 + 1);
	ok(NULL != tmp, "Malloc - Basic 1 - Basic tiny allocation");
	tmp[0] = 'A';
	memset(tmp + 1, 'B', TINY_MAX_SIZE / 2 - 1);
	...
	tmp = malloc(SMALL_MAX_SIZE / 2 + 1);
	ok(NULL != tmp, "Malloc - Basic 3 - Basic small allocation");
	memset(tmp, 'B', SMALL_MAX_SIZE / 2 - 1);
	tmp[SMALL_MAX_SIZE / 2] = '\0';
	...
	ok(malloc(0) != NULL, "Malloc - Basic 9 - Malloc(0)");
	ok(malloc(293828371837313123) == NULL, "Malloc - Basic 10 - Malloc with too large number");
}
...
static void calloc_tests()
{
	...
	ok(NULL == calloc(-1, sizeof(int)), "Calloc - error 1 - negative nmemb overflow");
	ok(NULL == calloc(1, -1), "Calloc - error 2 - negative size overflow");
	ok(NULL == calloc(2, -1), "Calloc - error 3 - negative size overflow");
	ok(NULL == calloc(SIZE_MAX, sizeof(int)), "Calloc - error 4 - overflow");
	ok(NULL == calloc(0, sizeof(int)), "Calloc - nmemb == 0");
	ok(NULL == calloc(1, 0), "Calloc - size == 0");
}
