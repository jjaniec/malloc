...
/*   Updated: 2019/09/18 16:35:32 by jjaniec          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <malloc.h>

extern void	*g_alloc_mem[2];
	...
	while (i < header->size)
	{
		...
		while (j-- && i < header->size)
		{
			...
			if (cur_byte_value < 10)
				ft_putchar('0');
			ft_putstr(ft_itoa_base(cur_byte_value, 16));
			if (i % 2)
				ft_putchar(' ');
			i += 1;
		}
...
static void	show_alloc_mem_list(char *type, t_malloc_header *start)
{
	while (start)
	{
		printf("%s block of size %u @%p\n", type, start->size, start);
